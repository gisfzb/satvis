...
import satIcon from "../images/icons/sat.svg?url";
...
export class SatelliteComponentCollection extends CesiumComponentCollection {
  ...
  createPoint() {
    // Use billboard with satellite icon instead of point
    const billboard = new BillboardGraphics({
      image: satIcon,
      horizontalOrigin: HorizontalOrigin.CENTER,
      verticalOrigin: VerticalOrigin.CENTER,
      scaleByDistance: new NearFarScalar(1e2, 0.6, 1e7, 0.1),
      color: Color.WHITE,
      scale: 1.2,
    });
  ...
  createGroundTrack() {
    ...
    // Get initial ground track to verify we have enough positions
    const initialTrack = this.props.groundTrack(this.viewer.clock.currentTime);
    if (initialTrack.length < 2) {
      // Not enough positions for corridor yet, skip
      return;
    }
